...
# The bake adds the openziti repo, but a Helm repository list is builder state
# rather than cluster state and does not survive into the shipped image: every
# booted VM starts with none. The upgrades below name a chart by repo alias, so
# without this they fail with "repo openziti not found" -- which is every VM on
# a port other than the baked default, since only those reach them.
ziti_repo() {
	helm repo list 2>/dev/null | grep -q '^openziti[[:space:]]' && return 0
	helm repo add openziti https://openziti.io/helm-charts >/dev/null
	helm repo update openziti >/dev/null
}

# The chart version a release is already on. Pinned explicitly because a repo
# added just now resolves to whatever is newest, and moving a port is not a
# request to upgrade the overlay underneath it.
chart_version() {
	helm list -n ziti --filter "^${1}\$" -o json 2>/dev/null |
		sed -n "s/.*\"chart\":\"${1}-\([^\"]*\)\".*/\1/p" | head -1
}
